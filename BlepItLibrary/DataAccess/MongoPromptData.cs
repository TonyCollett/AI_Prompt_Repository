...
public class MongoPromptData : IPromptData
{
    ...
    public async Task ReportPromptAsync(Prompt prompt, Report report)
    {
        var existingReport = _promptCollection.FindAsync(p => p.Id == prompt.Id && p.Reports.Any(r => r.IpAddress == report.IpAddress)).Result.FirstOrDefault();
        if (existingReport == null)
        {
            prompt.Reports.Add(report);
            await _promptCollection.ReplaceOneAsync(p => p.Id == prompt.Id, prompt);
            _cache.Remove(CacheName);
        }
    }
